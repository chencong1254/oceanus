...
public class HDMIInputService extends TvInputService {
  ...
  private static final String TAG = "MtkTvInput(" + HDMIInputService.class.getSimpleName() + ")";
  ...
  private void initHdmiControl() {
    ...
    if (mHdmiControl != null) {
      ...
      Log.e(TAG, "Can't access HdmiControlService");
    }
  }
  ...
  public Session onCreateSession(String inputId) {
    ...
    if (isLogicalInput(info)) {
      Log.d(TAG, "onCreateSession isLogicalInput");
      session = new HdmiLogicalInputSessionImpl(this, info);
    } else {
      Log.d(TAG, "onCreateSession isHardwareInput");
  ...
  public TvInputInfo onHardwareAdded(TvInputHardwareInfo hardwareInfo) {
    ...
    try {
      ...
      Log.e(TAG, "onHardwareAdded:" + "HDMI " + hardwareInfo.getHdmiPortId());
  ...
  public String onHardwareRemoved(TvInputHardwareInfo hardwareInfo) {
    ...
    if (DEBUG)
      Log.d(TAG, "onHardwareRemoved returns " + inputId);
    return inputId;
  }
  ...
  public TvInputInfo onHdmiDeviceAdded(HdmiDeviceInfo device) {
    ...
    if (mCecInputIdMap.indexOfKey(id) >= 0) {
      Log.e(TAG, "Already created TvInputInfo for id:" + id);
      return null;
    }
    int portId = device.getPortId();
    if (portId < 0) {
      Log.e(TAG, "Failed to get HDMI port for id: " + id);
      return null;
    }
    if (mHardwareIdMap.indexOfKey(portId) < 0) {
      Log.e(TAG, "Unknown HDMI port " + portId + " for id=" + id);
      return null;
    }
    ...
    TvInputInfo info = null;
    try {
      Log.d(TAG, "onHdmiDeviceAdded DisplayName:" + device.getDisplayName()
    ...
    mInputMap.put(info.getId(), info);
    if (DEBUG)
      Log.d(TAG, "onHdmiDeviceAdded returns " + info.getId());
    return info;
  }
  ...
  public String onHdmiDeviceRemoved(HdmiDeviceInfo device) {
    ...
    if (inputId == null) {
      if (DEBUG) {
        Log.d(TAG, "TvInputInfo for id=" + id + " does not exist.");
      }
      return null;
    }
    ...
    mInputMap.remove(inputId);
    if (DEBUG) {
      Log.d(TAG, "onHdmiDeviceRemoved port=" + device.getPortId());
      Log.d(TAG, "onHdmiDeviceRemoved returns " + inputId);
    }
    return inputId;
  }
  ...
  private abstract class HdmiInputSessionImpl extends Session {
    ...
    HdmiInputSessionImpl(Context context, TvInputInfo info) {
      ...
      Log.d(TAG, "HdmiInputSessionImpl() totalCount=" + totalCount);
    ...
    private void acquireHardware() {
      if (mHardware != null) {
        Log.d(TAG, "acquireHardware(mHardware != null) return");
        return;
      }
      ...
      {
        Log.d(TAG, "acquireHardware() activeOutType = TvInputConst.InputInvalid directly return");
        return;
      }
      Log.d(TAG, "acquireHardware() deviceId=" + deviceId);
    ...
    private boolean postSetSurface(Surface surface, TvStreamConfig config) {
      ...
      Log.d(TAG, "postSetSurface() send DO_SET_SURFACE message, return true");
      // boolean ret = mHardware.setSurface(surface, config);
      // Log.d(TAG, "setSurface() setSurface return = " + ret);
      if (mSourceVolume != -1f)
      {
        Log.d(TAG,
    ...
    protected boolean setSurface(Surface surface) {
       ...
       */
      Log.d(TAG, "setSurface " + ", mReleased = " + mReleased);
      if (mReleased) {
        Log.e(TAG, "setSurface, (mReleased) return false");
        return false;
      }
      if (mHardware == null) {
        acquireHardware();
        if (mHardware == null) {
          Log.d(TAG, "setSurface() setSurface (mHardware == null)return false ");
          return false;
        }
      }
      ...
      if (surface != null) {
        ...
        if (config == null) {
          Log.d(TAG, "setSurface() setSurface (config == null)return false ");
          return false;
        }
      }
      // boolean ret = mHardware.setSurface(surface, config);
      // Log.d(TAG, "setSurface() setSurface return = " + true);
      if (config == null)
      {
        Log.d(TAG, "setSurface() postSetSurface surface = " + surface + ", config = null");
      }
      else
      {
        Log.d(TAG,
    ...
    public void onRelease() {
      ...
      if (mInputContext.surface != null){
        if (mHardware != null) {
          ...
          mHandler.obtainMessage(ServiceHandler.DO_SET_SURFACE, args).sendToTarget();
          if (DEBUG)
            Log.d(TAG, "onRelease() send DO_SET_SURFACE(null) message");
        }
      }
      
      if (DEBUG) {
        Log.d(TAG, "onRelease, call startInternalSourceSelectTimer");
      }
      mHandler.clearTimer(TIMER_INTERNAL_SELECT);
      startInternalSourceSelectTimer();
      
      if (DEBUG)
        Log.d(TAG, "onRelease() call mInputContext.reset()");
      mInputContext.reset();
    ...
    public void onSetMain(boolean isMain) {
      if (DEBUG) {
        Log.d(TAG, "onSetMain: isMain = " + isMain + ", mReleased = " + mReleased);
      }
      if (mReleased) {
        Log.e(TAG, "onSetMain, (mReleased) return");
        return;
      }
      mHandler.clearAllTimers();
      if (DEBUG) {
        Log.d(TAG, "onSetMain: " + isMain + " info:" + mInfo.getId() + " selectedInput:"
      ...
      if (isMain) {
        if (DEBUG) {
          Log.d(TAG, "onSetMain: isMain = true");
        }
        if (mInputContext.state == STATE_INIT) {
          if (DEBUG) {
            Log.d(TAG, "onSetMain:isMain = true(mInputContext.state == STATE_INIT)"
                + " call hdmiSelect");
          }
          hdmiSelect();
        }
        ...
        if (DEBUG) {
          Log.d(TAG, "onSetMain:isMain = false call startInternalSourceSelectTimer");
        }
        startInternalSourceSelectTimer();
      }
    }
    ...
    public void onSetStreamVolume(float volume) {
      ...
      try{
        if (mHardware != null)
        {
          Log.d(TAG, "onSetStreamVolume (mHardware != null) call mHardware.setStreamVolume(" + volume
              + ")");
    ...
    public boolean onTune(Uri channelUri) {
      ...
      synchronized (TvInputService.class) {
        ...
        if ((activeOutType == TvInputConst.InputInvalid) || (activeOutType != newOutType)) {
          ...
          Log.d(TAG, "onTune, same to old one, activeOutType = " + this.activeOutType);
        }
        // needSetSurface = true;
      }

      if (mInputContext.surface != null) {
        Log.d(TAG, "onTune call (mInputContext.surface != null) HdmiInputSessionImpl::setSurface");
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
      if (mHdmiControl != null) {
        if (keyCode == KeyEvent.KEYCODE_MENU) {
          mHdmiControl.sendKeyEvent(176, false);
        } else {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
      if (mHdmiControl != null) {
        if (keyCode == KeyEvent.KEYCODE_MENU) {
          mHdmiControl.sendKeyEvent(176, true);
        } else {
    ...
    public void handleMessage(Message msg) {
      ...
      switch (msg.what) {
        case TIMER_DEVICE_SELECT:
          ...
          if (DEBUG)
            Log.d(TAG, "Time out on device select.");
          session = getSession((String) msg.obj);
          if (session != null) {
            if (DEBUG)
              Log.d(TAG, "Time out on device select. notifyVideoUnavailable");
                    session.notifyVideoUnavailable(TvInputManager.VIDEO_UNAVAILABLE_REASON_UNKNOWN);
          }
          break;
        case TIMER_INTERNAL_SELECT:
          if (DEBUG)
            Log.d(TAG, "Internal input is selected.");
          if (mHdmiControl == null) {
            if (DEBUG)
            {
              Log.d(TAG, "Internal input is selected.(mHdmiControl == null) break;");
            }
            break;
          }
          if (DEBUG)
        ...
        case DO_SET_SURFACE:
          ...
          {
            Log.d(TAG, "DO_SET_SURFACE()  surface = " + surface + ", config = "
                + config.toString());
          }
          boolean ret = false;
          try{
          ...
          if (ret) {
            if (surface != null) {
              Log.d(TAG, "DO_SET_SURFACE mHardware.setSurface() notifyVideoAvailable");
              session.notifyVideoAvailable();
            }
          } else {
            if (surface != null) {
        ...
        case DO_ADD_SESSION:
          ...
          Log.d(TAG, "DO_ADD_SESSION. add inputId = " + inputId + ",session = " + session);
          mSessionMap.put(inputId, session);
          break;
        default:
  ...
  private class HdmiLogicalInputSessionImpl extends HdmiInputSessionImpl {
    ...
    public boolean onSetSurface(Surface surface) {
      ...
      if (surface == null || mHdmiControl == null) {
        Log.d(TAG, "onSetSurface surface == null || mHdmiControl == null");
      ...
      if ((mInputContext.state == STATE_DONE)
        ...
        if (DEBUG)
          Log.d(TAG, "onSetSurface(mInputContext.state == STATE_INIT) call deviceSelect");
        deviceSelect();
      ...
      mHdmiControl.deviceSelect(mId, new SelectCallback() {
        ...
        public void onComplete(final int result) {
          runOnServiceThread(new Runnable() {
            ...
            public void run() {
              if (DEBUG) {
                Log.d(TAG, "deviceSelect onComplete: result=" + result);
              }
              if (result != HdmiControlManager.RESULT_SUCCESS) {
                if (DEBUG) {
                  Log.d(TAG, "deviceSelect onComplete: notifyVideoUnavailable");
                }
              ...
              if (isMhlDevice()) {
                if (DEBUG) {
                  Log.d(TAG, "deviceSelect onComplete: isMhlDevice");
                }
                if (mInputContext.surface != null) {
                  mInputContext.state = STATE_DONE;
                  setSurface(mInputContext.surface);
                  ...
                  if (DEBUG) {
                    Log.d(TAG, "deviceSelect onComplete: isMhlDevice notifyVideoAvailable");
                  }
  ...
  private class HdmiHardwareInputSessionImpl extends HdmiInputSessionImpl {
    ...
    public boolean onSetSurface(Surface surface) {
      ...
      if (surface != null) {
        if (mInputContext.state == STATE_INIT) {
          if (DEBUG)
            Log.d(TAG, "onSetSurface(mInputContext.state == STATE_INIT) call portSelect");
          portSelect();
        }
      }
      mInputContext.surface = surface;
      if (DEBUG)
        Log.d(TAG, "onSetSurface HdmiInputSessionImpl setSurface");
      return setSurface(surface);
    }
      ...
      mHdmiControl.portSelect(mPortId, new SelectCallback() {
        ...
        public void onComplete(final int result) {
          runOnServiceThread(new Runnable() {
            ...
            public void run() {
              if (DEBUG)
                Log.d(TAG, "portSelect onComplete: result=" + result);
              if (result != HdmiControlManager.RESULT_SUCCESS) {
                if (DEBUG) {
                  Log.d(TAG, "portSelect onComplete: notifyVideoUnavailable");
                }
                notifyVideoUnavailable(TvInputManager.VIDEO_UNAVAILABLE_REASON_UNKNOWN);
              } else {
                if (DEBUG) {
                  Log.d(TAG, "portSelect onComplete: notifyVideoAvailable");
                }
                notifyVideoAvailable();
              }
