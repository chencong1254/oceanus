 ...
 */
public class CompositeInputService extends AbstractInputService {
  private final List<AbstractRutineTaskThread> syncThreads =
                                 new ArrayList<AbstractRutineTaskThread>();
  // private ContentResolver contentResolver;
  private CompositeInputSessionImpl compositeInputSessionImpl;
  private Timer timer;
  private int index = 1;
  ...
  class CreateThreadTask extends TimerTask {
    ...
    public void run() {
      ...
      if (tvRemoteServiceOK) {
        syncThreads.add(new CompositeInputRutineTaskThread("CompositeInputRutineTaskThread",
            contentResolver, CompositeInputService.this));
  ...
  public Session onCreateSession(String inputId) {
    ...
    compositeInputSessionImpl = new CompositeInputSessionImpl(this, info, mHardwareDeviceId, index);
    ...
    // mSessionMap.put(inputId, compositeInputSessionImpl);
    SomeArgs args = SomeArgs.obtain();
    args.arg1 = inputId;
    args.arg2 = compositeInputSessionImpl;
    ...
    index = index + 1;
    return compositeInputSessionImpl;
  }

  public CompositeInputSessionImpl getCompositeInputSession() {
    return compositeInputSessionImpl;
  }
   ...
   * VGA <BR>
   */
  class CompositeInputSessionImpl extends AbstractInputSession {
    protected CompositeInputSessionImpl(AbstractInputService mtkTvCommonInputService,
  ...
  class RutineTaskCallbackHandler extends MtkTvTVCallbackHandler {
    ...
    private final CompositeInputRutineTaskThread mRutineThread;

    public RutineTaskCallbackHandler(CompositeInputRutineTaskThread rutineThread) {
  ...
  public class CompositeInputRutineTaskThread extends AbstractRutineTaskThread {
    public String TAG = "CompositeInputRutineTaskThread";

    private Handler mHandler;

    private final CompositeInputService service;
    ...
    public CompositeInputRutineTaskThread(String name, ContentResolver contentResolver,
        CompositeInputService service) {
      super(name, contentResolver);
      this.service = service;
      Log.d(TAG, "constructed function");

      ratingProcesser = new TVInputRatingProcesser("CompositeInputService");
    ...
    private boolean checkSessionexist() {
      ...
      if (service == null || service.getCompositeInputSession() == null) {
        Log.d(TAG, "checkSessionexist session is null");
        return false;
      }

      ratingProcesser.setTVInputService(service);
      ratingProcesser.setTVInputSession(service.getCompositeInputSession());
